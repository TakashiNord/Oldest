...
static int GetDInColors(int BitCount)
{
	switch( BitCount )
	{
	case 1:case 3:case 4:	case 8:return 1 << BitCount;
	default:	return 	0;
	}
}
...
if ( (State & (BS_MOUSEDOWN | BS_KEYDOWN)) && !GetState(BS_MOUSEUPDOWN) )
 ...
 if ( GetState(BS_FOCUS) )  hBits = (HBITMAP)FocUpBits;
  else  hBits = (HBITMAP)UpBits;
...
// Draw the button border
{
 ...
 // Select brush
 if ( GetState(BS_DEFAULT) ) hBorderBrush = GetStockObject(BLACK_BRUSH);
  else	hBorderBrush = GetStockObject(WHITE_BRUSH);
hOldBrush = SelectObject(hDC, hBorderBrush);
 ...
 PatBlt(hDC, Frame.left, Frame.bottom - BD_BORDERWIDTH, Width,BD_BORDERWIDTH, PATCOPY);
 PatBlt(hDC, Frame.right - BD_BORDERWIDTH, Frame.top, BD_BORDERWIDTH,Height, PATCOPY);
// Clean up scope
...
// Draw the bitmap
{
 ...
 if ( GetState(BS_DISABLED) )
 {
  ...
  lbLogBrush.lbHatch = (int)LoadBitmap(hInstance,MAKEINTRESOURCE(BT_DISABLEBITS));;
...
static void Repaint(HWND hWnd)
{
  ...
  Paint(hWnd, hDC);ReleaseDC(hWnd, hDC);
}
...
static void Set_State(HWND hWnd, int AState, BOOL Enable)
{
 ...
 if (NewState != OldState) { SetWord(OF_STATE, NewState);Repaint(hWnd);	}
}
